    ...
    def _crawl_image(self, tag):
        ...
        for post in posts:
            ...
            if cursor_result == None:
                img = self.opener.open(url+file_url)
                img_file = open(self.save_dir
                              + '/' 
                              + file_url.split('/')[-1], 'wb')
                img_file.write(img.read())
                logging.info('저장 완료 {0}'.format(file_url))
            else:
                logging.info('중복 {0}'.format(file_url))
            # 데이터베이스 변수
            post_id = str(post['id'])
            ...
            # 데이터베이스 저장
            self.cursor.execute('UPDATE OR IGNORE image SET '
                         ...
                         + 'WHERE '
                         + 'id=' + post_id)
            self.cursor.execute('INSERT OR IGNORE INTO image '
