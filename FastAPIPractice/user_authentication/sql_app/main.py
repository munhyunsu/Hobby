...
from .conf import SECRET_KEY, ALGORITHM, ACCESS_TOKEN_EXPIRE_MINUTES
...
@app.post('/users/ops/', response_model=schemas.Op)
def create_op_for_user(
    background_tasks: BackgroundTasks,
    current_user: Annotated[schemas.User, Depends(get_current_active_user)],
    op: schemas.OpCreate,
    db: Session = Depends(get_db)
):
    db_op = crud.create_user_op(db=db, op=op, user_id=current_user.id)
    background_tasks.add_task(process_task, db_op, db)
    return db_op

@app.get('/users/ops/', response_model=list[schemas.Op])
def read_op_of_user(
    current_user: Annotated[schemas.User, Depends(get_current_active_user)],
    db: Session = Depends(get_db)
):
    return crud.get_ops_of_user(db, current_user.id)
...
@app.get('/users/me/items/', response_model=list[schemas.Op])
def read_own_items(
    current_user: Annotated[schemas.User, Depends(get_current_active_user)],
    db: Session = Depends(get_db)
):
